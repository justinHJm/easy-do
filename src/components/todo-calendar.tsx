...
export function TodoCalendar({ value, minimumDate, onConfirm, onCancel }: {
  value?: string; minimumDate?: string; onConfirm: (value?: string) => void; onCancel: () => void;
}) {
    ...
    <View>
      ...
      <View style={styles.grid}>
        {calendarCells(year, monthIndex).map((day, index) => {
          ...
          const disabled = !!minimumDate && key < minimumDate;
          return <View key={index} style={styles.cell}>{day !== null && (
            <Pressable accessibilityRole="button" accessibilityLabel={`${fullDate(key)}${key === today ? ', 오늘' : ''}`}
              accessibilityState={{ selected: selected === key, disabled }} disabled={disabled} onPress={() => setSelected(key)}
              style={[styles.day, key === today && styles.today, key === selected && styles.selected, disabled && styles.disabled]}>
              <Text style={[styles.dayText, key === selected && styles.white, disabled && styles.disabledText]}>{day}</Text>
            </Pressable>
          )}</View>;
        })}
      </View>
      <Text style={styles.hint}>테두리: 오늘 · 초록 배경: 선택한 날짜{minimumDate ? ` · ${fullDate(minimumDate)} 이후 선택` : ''}</Text>
...
const styles = StyleSheet.create({
  ...
  dayText: { color: Colors.light.text, fontSize: 15 }, white: { color: '#FFFFFF' }, disabled: { opacity: 0.35 }, disabledText: { color: Colors.light.textSecondary },
