...
현재 eas.json, android.package, EAS projectId, android 폴더가 없다. npm run android는 개발 서버이며 APK 빌드가 아니다.
실제 빌드는 build-apk에서 계정·식별자·서명·업로드 범위를 확인한다. 이번 설정 변경으로 빌드나 서버 종료를 하지 않는다.
